...
function adaptive_header(w,h) {
		...
		var headerLang=$('.header_top_lenguages');
	if(w<767){
		...
		if(headerLang.hasClass('done')){
			headerLang.removeClass('done').prependTo($('.header_top'));
		}
	}
	if(w<767){
		if(!$('.header_bottom_column_menu').hasClass('done')){
			$('.header_bottom_column_menu').addClass('done').appendTo(headerMenu);
		}
	}else{
		$.each($('.header_bottom_column_menu'), function(index, val) {
			if($(this).hasClass('header_bottom_column_menu-RIGHT')){
				if($(this).hasClass('done')){
					$(this).removeClass('done').prependTo($('.header_bottom_column').eq(2));
				}
			}else{
				if($(this).hasClass('done')){
					$(this).removeClass('done').prependTo($('.header_bottom_column').eq(0));
				}
			}
		});
	}
